...
function stableCallId(toolName: string, input: Record<string, unknown>): string {
...
export interface HookOptions extends ClientOptions {
  ...
  claimOnRead?: boolean;
  onConflict?: ConflictMode;  // default: "warn" (ignored if server has locking=true)
}
...
function formatMailboxMessage(msg: MailboxMessage): string {
...
async function checkCriticalMailbox(
  ...
  try {
    ...
    return msgs.filter(
      (m) => m.type === "yield_request" || m.type === "escalation"
    );
  } catch {
    return [];
  }
...
export function createHooks(opts: HookOptions) {
  const activeClaims = new Map<string, string>(); // callId -> claimId
  const heartbeats = new Map<string, ReturnType<typeof setInterval>>(); // claimId -> timer

  const startHeartbeat = (claimId: string, ttlSeconds: number) => {
    // Default is true; skip only if explicitly false
    if (opts.autoHeartbeat === false) return;
  ...
  const stopHeartbeat = (claimId: string) => {
  ...
  const toolToOp = (toolName: string): Operation | null => {
    switch (toolName) {
  ...
  const hooks = {
    ...
    async preToolUse(toolName: string, input: Record<string, unknown>): Promise<PreToolUseResult> {
      ...
      if (!uri) return { permissionDecision: "allow" };

      const op = toolToOp(toolName);
      if (!op) return { permissionDecision: "allow" };
      if (op === "read" && !opts.claimOnRead) return { permissionDecision: "allow" };
      ...
      try {
        ...
        if ((resp as any).error === "resource_locked") {
          const lockedBy = (resp as any).locked_by ?? "another agent";
          const lockedIntent = (resp as any).intent ?? "unknown intent";
          return {
            permissionDecision: "deny",
